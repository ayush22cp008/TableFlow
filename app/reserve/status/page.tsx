  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100">
      ...
      <main className="max-w-2xl mx-auto px-4 py-10">
        <h1 className="text-2xl font-extrabold tracking-tight mb-6">Check Reservation Status</h1>
        ...
        <form onSubmit={searchRequests} className="flex gap-2 mb-8">
          <input 
            ...
            className="flex-1 px-4 py-2.5 bg-gray-800/50 border border-gray-700 rounded-xl text-white outline-none focus:ring-2 focus:ring-accent-indigo"
          />
          <button type="submit" disabled={!name || loading} className="px-6 py-2.5 bg-accent-indigo hover:bg-accent-indigo-hover text-white rounded-xl font-medium disabled:opacity-50">
            Search
          </button>
        </form>
              ...
              <div key={req.id} className="p-4 bg-surface border border-surface-border rounded-card shadow-card">
                ...
                {req.status === 'approved' && req.unique_code && (
                  <div className="mt-4 p-3 bg-indigo-900/30 border border-indigo-500/30 rounded-lg text-center">
                    ...
                    <p className="text-2xl font-mono font-bold tracking-widest text-accent-amber">{req.unique_code}</p>
                  </div>
                )}
              </div>
            ))}
