  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100">
      ...
      <main className="max-w-5xl mx-auto px-4 py-10">
        <h1 className="text-2xl font-extrabold tracking-tight mb-6">Analytics</h1>

        {/* KPI Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          {[
            { label: "Today's Revenue", value: `₹${todayRevenue.toFixed(2)}`, sub: 'net of cancelled', color: 'text-accent-amber' },
            ...
            <div key={kpi.label} className="p-5 bg-surface border border-surface-border rounded-card shadow-card">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="p-5 bg-surface border border-surface-border rounded-card shadow-card">
            <h2 className="font-semibold text-white mb-4">Revenue — Last 7 Days</h2>
            <div className="flex items-end gap-2 h-40">
              {weekData.map((day) => (
                <div key={day.date} className="flex-1 flex flex-col items-center gap-1">
                  <span className="text-xs text-accent-amber">{day.revenue > 0 ? `₹${Math.round(day.revenue)}` : ''}</span>
                  <div
