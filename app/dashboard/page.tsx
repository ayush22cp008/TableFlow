    ...
    <div className="min-h-screen bg-gray-950 text-gray-100">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="mb-8">
          ...
          <p className="text-gray-400 mt-1">Welcome back, <span className="text-accent-indigo">{user?.email}</span></p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[
            ...
            <Link
              key={card.href}
              href={card.href}
              className="p-6 bg-surface border border-surface-border rounded-card shadow-card hover:border-accent-indigo/50 transition-all group"
            >
              <div className="text-3xl mb-3">{card.icon}</div>
              <h3 className="font-semibold text-white mb-1 group-hover:text-accent-indigo transition-colors">{card.label}</h3>
